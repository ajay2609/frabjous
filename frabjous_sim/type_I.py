...
def generate_frb(frb_dict, snr, add_noise=True):
    """
    Generate a simulated FRB dynamic spectrum.

    Parameters
    ----------
    frb_dict : dict
        FRB parameters (DM, SM, components).
    snr : float
    ...
    for comp in frb_dict["components"]:
        ...
        comp_ts = apply_spectral_index_and_running(
            comp_ts,
            comp_ts.shape,
            comp.get("spectral_index", 0.0),
            comp.get("spectral_running", 0.0),
        )
...
def simulate_frbs(config_file):
    ...
    # --- Width distribution ---
    # 20% uniform + 80% lognormal (CHIME/FRB Catalog I)
    ...
    for i in range(count):
        ...
        img = generate_frb(frb_dict, snr, params["add_noise"])
